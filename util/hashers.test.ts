...
const hexEncoded = encodeHex(encoded)

type HasherKind = keyof typeof hashes
...
for (const hasherKind in hashes) {
  Deno.test(hasherKind, async () => {
    ...
    assertEquals(encodeHex(hasher.hash(encoded)), hash)
    const hashData = await hasher.$hash($foo).encodeAsync(foo)
    assertEquals(encodeHex(hashData), hash)
    if (hasher.concat) {
